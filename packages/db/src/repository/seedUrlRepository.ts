...
async function getUrlsCrawled(
  seedUrlId: string | Types.ObjectId,
): Promise<{ urlCrawled: UrlCrawledType[] } | null> {
...
async function getAnalyzedData(
  seedUrlId: string | Types.ObjectId,
): Promise<{ analyzedData: analyzedDataType } | null> {
  return await seedUrlModel
    .findById(seedUrlId)
...
async function updateAnalyzedData(
  seedUrlId: string | Types.ObjectId,
  analyzedDataId: string | Types.ObjectId,
) {
...
async function updateSeedUrlStatus(
  seedUrlId: string | Types.ObjectId,
  status: SeedUrlType["status"],
) {
  return await seedUrlModel.findByIdAndUpdate(seedUrlId, { status }, { new: true });
}
...
export async function updateUrlCrawled(
  seedUrlId: string | Types.ObjectId,
  urlCrawledId: string | Types.ObjectId,
) {
  return await seedUrlModel.findByIdAndUpdate(
    seedUrlId,
    { $push: { urlCrawled: urlCrawledId } },
    { new: true },
  );
}
