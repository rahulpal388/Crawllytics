...
export async function getIPLocation(ip: string): Promise<IPLocationType | null> {
  try {
    ...
    console.error(`IP geolocation failed for ${ip}:`, error);

    return null;
  }
}

export async function getLocationByIP(ip: string | null): Promise<string | null> {
  if (!ip) {
    return null;
  }
