...
} from "@repo/contracts/types/crawl/domain-leve-information/domainInformation.Types";
...
export async function getDomainInfo(
    ...
    try {
        ...
        const service =
            bootstrapResponse.data.services.find(
                ([tlds]) =>
                    tlds.some(
                        (item) =>
                            item.toLowerCase() === tld
                    )
            );
        ...
        console.error(
            `Failed to get domain information for ${domain}`
        );
