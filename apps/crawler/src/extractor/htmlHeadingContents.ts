...
export function htmlContentsExtractor($: cheerio.CheerioAPI): HTMLHeadingContentsType {
  ...
  const headingCount = h1Contents.length + h2Contents.length + h3H6Contents.length;
  ...
  $("h1,h2,h3,h4,h5,h6").each((_, el) => {
    ...
    if (
      !nextElement ||
      !["p", "ul", "ol", "div", "section", "article"].includes(nextElement.tagName.toLowerCase())
    ) {
  ...
  const headingToContentRatio =
    wordCount === 0 ? 0 : Number((headingWordCount / wordCount).toFixed(3));
