...
export function mobileUIUX($: cheerio.CheerioAPI): MobileUIUXType {
  ...
  const viewportContent = metaViewport.attr("content")?.toLowerCase() ?? "";

  const viewportIsMobileReady = viewportContent.includes("width=device-width");
  ...
  const fontScalingSupported =
    !viewportContent.includes("user-scalable=no") && !viewportContent.includes("maximum-scale=1");
  ...
  const estimatedPopupPresence = $("[id],[class]")
    ...
    .some((element) => {
      ...
      return popupKeywords.some((keyword) => value.includes(keyword));
    });
  ...
  $("[style]").each((_, element) => {
    ...
    if (widthMatch && Number(widthMatch[1]) > 1200) {
