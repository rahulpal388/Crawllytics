...
import { ResourceHintType, ResourceHintRelType } from "@repo/config/types/urlInformationType/htmlHeaderResponseTypes";


export function getResourceHintData(
    $: cheerio.CheerioAPI,
    baseUrl: URL
): ResourceHintType[] {
    ...
    $("link[rel]").each((_, element) => {
        ...
        resourceHints.push({
            ...
            crossOrigin:
                ($(element).attr("crossorigin") || null)?.trim() || null,
            ...
            imageSrcSet:
                ($(element).attr("imagesrcset") || null)?.trim() || null,

            imageSizes:
                ($(element).attr("imagesizes") || null)?.trim() || null,
