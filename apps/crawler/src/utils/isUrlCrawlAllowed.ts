...
export function isUrlCrawlAllowed(
    url: string,
    ...
    const matchingGroups = robotsTxt.filter(group =>
        group.userAgent.includes("*") ||
        group.userAgent.some(
            ua => ua.toLowerCase() === USER_AGENT.toLowerCase()
        )
    );
    ...
    for (const group of matchingGroups) {
        ...
        for (const allowRule of group.allow) {
            if (
                allowRule &&
                pathname.startsWith(allowRule) &&
                allowRule.length > longestMatch.length
            ) {
