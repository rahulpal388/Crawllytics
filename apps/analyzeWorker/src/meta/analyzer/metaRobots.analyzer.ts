...
export function analyzeMetaRobots(directives: string[]): MetaRobotsAnalysis {
  console.log("analyzeMetaRobots directives", directives);
  const values = new Set(directives.map((v) => v.toLowerCase()));
  ...
  return {
    ...
    hasMaxImagePreview: directives.some((v) => v.startsWith("max-image-preview")),
    hasMaxVideoPreview: directives.some((v) => v.startsWith("max-video-preview")),
    hasUnavailableAfter: directives.some((v) => v.startsWith("unavailable_after")),
