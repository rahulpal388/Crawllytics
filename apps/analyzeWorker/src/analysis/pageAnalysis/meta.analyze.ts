...
export function metaAnalyze(metaInfo: GatherInfoType["htmlHeader"]): PageMetaAnalysis {
    ...
    const duplicateTitle = title.map(t => t.text).some((text, index, arr) => arr.indexOf(text) !== index);
    ...
    const duplicateMetaDescription = metaDescription.map(md => md.text).some((text, index, arr) => arr.indexOf(text) !== index);
