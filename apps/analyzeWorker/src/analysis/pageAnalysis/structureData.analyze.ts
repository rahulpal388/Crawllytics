...
export function structureDataAnalysis(data: GatherInfoType["structureData"]): StructuredDataAnalysis {
    ...
    const validSchemas = data.JsonLdBlocks.map(block => block.isValid).filter(isValid => isValid).length;
