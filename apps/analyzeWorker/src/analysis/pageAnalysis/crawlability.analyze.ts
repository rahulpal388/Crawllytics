...
export function crawlability(gatherInfo: GatherInfoType): PageCrawlabilityAnalysis {
  const isReachable =
    gatherInfo.networkInfo.statusCode >= 200 && gatherInfo.networkInfo.statusCode < 400;
  const isBlockedByRobots = gatherInfo.urlAnalyses.isBlockedByRobotsTxt;
  const isBlockedByMetaRobots =
    gatherInfo.htmlHeader.meta.metaRobot.includes("noindex") ||
    gatherInfo.htmlHeader.meta.metaRobot.includes("nofollow");

  const isBlockedByXRobots =
    gatherInfo.networkInfo.responseHeaders.xRobotsTag.includes("noindex") ||
    gatherInfo.networkInfo.responseHeaders.xRobotsTag.includes("nofollow");
